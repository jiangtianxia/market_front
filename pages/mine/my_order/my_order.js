...
Page({
  ...
  getOrderList(firstFlag) {
    ...
    wx.request({
      method: 'GET',
      url: 'http://127.0.0.1:8888/market/api/v1/order/list?to_openid='+openid +'&status=' + that.data.status,
      timeout: 30000,
      header: {
  ...
  pay(event) {
    ...
    wx.showModal({
      title: '提示',
      content: '是否支付商品价格' + this.data.orderList[index].total_price + '元',
      confirmText: '支付'
    })
    .then(res=>{
      if(res.confirm == true) {
        this.updateOrder(this.data.orderList[index].order_id, 2)
      } else {
        wx.showToast({
          icon: 'error',
          title: '支付失败',
  ...
  confirmReceipt(event) {
    ...
    .then(res=>{
      if(res.confirm == true) {
        this.updateOrder(this.data.orderList[index].order_id, 4) 
      } else {
        wx.showToast({
          icon: 'error',
          title: '更新订单失败',
